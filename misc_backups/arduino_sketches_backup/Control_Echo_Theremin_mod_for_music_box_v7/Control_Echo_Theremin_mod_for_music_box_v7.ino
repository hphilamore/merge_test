...
NewPing sonar(left_trigPin, left_echoPin, MAX_DISTANCE); // NewPing setup of pins and maximum distance.
...
//  int string_freq = 1023 - (1023 * distance/50);

  delayMicroseconds(3000);                     // Wait 50ms between pings (about 20 pings/sec). 29ms should be the shortest delay between pings.
  Serial.print("Ping: ");
...
//  Serial.println(knob);
  int averaged = kAverage.next(bumpy_input);
  int average_knob = kAverage.next(knob);
//  int average_duration = kAverage.next(string_freq);
  //int average_duration = kAverage.next(duration);
//  Serial.println(distance);
//  Serial.println(string_freq);
//  aSin0.setFreq(averaged);
  //aSin0.setFreq(average_duration);   // comment aSin* out to change echo 
  aSin0.setFreq(average_knob);   // comment aSin* out to change echo 
