...
BOT_DEFAULTS = OrderedDict([
    ("base_metabolism", 0),
    ("e_consumed_per_feed", 12),
    ("e_stored_initial", 5),
    ("e_cost_per_step", 1),
    ("e_threshold_store", 7),
    ("e_threshold_explore", 12),
    ("e_threshold_trophallaxis", 0),
    ("e_threshold_full", 0),
    ("sense_range", 1),
    ("transmission_range" , 2),
    ("trophallaxis", use_trophallaxis),
    ("trophallaxis_mode", "top_up_recipiant_cautious"),
    # options:
    # "equalise",
...
class Food(object):
    ...
    def _designed_map(self):
        ...
        #food_area = slice(11:19, 11:19)
        self.food_map[11:19, 11:19] = 1  # area covered by food
        self.food_map[a:b, a:b] = 1  # area covered by food
        #self.food_map[food_area]
...
class Bot(object):
    ...
    def __init__(self, world, *, base_metabolism, e_consumed_per_feed,
                 ...
                 e_threshold_full, sense_range,
                 transmission_range, trophallaxis,
                 trophallaxis_mode, relay, e_stored_max,
                 energy_conversion_efficiency,
                 time_steps_to_charge):
        ...
        self.new_location_generators = {
            "new_location_random": self.new_location_random,
            "new_location_towards_neighbour": self.new_location_towards_neighbour,
            "new_location_towards_signal": self.new_location_towards_signal,
            "step_to_explore": self.new_location_towards_signal,
            "step_towards_feeder": self.new_location_towards_signal,
            }
    ...
    def evaluate_neighbours(self, world, location, neighbours):
        ...
        # neighbours_not_on_food = (np.ma.make_mask(world.bot_map)*1 - \
        #                          np.ma.make_mask(world.food_map)*1)\
        #                          - np.ma.make_mask(world.explore_mode_map)*1

        # locations of neighbours, INcluding those in Explore mode
        neighbours_not_on_food =  np.ma.make_mask(world.bot_map) * 1 - \
                                  np.ma.make_mask(world.food_map) * 1
                                 #- np.ma.make_mask(world.explore_mode_map) * 1

        neighbours_on_food =    np.ma.make_mask(world.bot_map) * 1 + \
                                np.ma.make_mask(world.food_map) * 1
                                #- np.ma.make_mask(world.explore_mode_map) * 1
        ...
        if local_energy:
            neighbours_less_food = self.sense(neighbours_not_on_food, "has_no_food")
            ...
            neighbours_same_food_full = self.sense(neighbours_on_food, "has_food")
            ...
            neighbours_same_food_none = self.sense(
                neighbours_not_on_food, "has_no_food")
            ...
            neighbours_more_food = self.sense(neighbours_on_food, "has_food")
            if neighbours_more_food:
                ...
                #     world.setup_move("step_to_explore",
                #                          location, self.relay_chain.heading)

    def new_location_random(self, location, target_location, world):
    ...
    def new_location_towards_neighbour(self, location,
    ...
    def new_location_towards_signal(self, location,
                                    target_location, world):
        """
        Determines the next space the bot will attempt to move to.
    ...
    def move(self, world, location):
        ...
        world.charge_mode_map[location] = 0
        ...
        if world.bot_map[new_location[::-1]]:
            # obstacle avoidance
            if (self.new_location_generator == "new_location_towards_signal" or
                ...
                # generate a new location
                new_location = self.new_location_generators[
                    "new_location_towards_neighbour"](location,
                                                obstacle,
                                                world)
        ...
        # if the new location is free...
            ...
            if bot_moved == 1:
                ...
                if np.ma.make_mask(world.food_map)[new_location[::-1]]:
                    self.stop_on_food(new_location, world)
    ...
    def update_map(self, new_location, initial_location,
                   _map, consume_energy, initial_location_zero):
    ...
    def stop_on_food(self, new_location, world):
        """
        ...
        if world.explore_mode_map[new_location[::-1]]:
            ...
            #
            #         world.setup_move("new_location_towards_signal",
            #                        location, relay_signal,
            #                        world.Estored_map[location[::-1]]
    ...
    #                                 world.Estored_map[self.location[::-1]]
    #                                 - BOT_DEFAULTS["e_threshold_store"])
...
#                # (np.ma.make_mask(world.Estored_map) * 1))
...
#
#         bots_with_energy = np.where(world.Estored_map
...
#                 world.Estored_map[recruit_initial_location[::-1]] = 0
...
class World(object):
    ...
    def __init__(self, n_bots, food):
        ...
        self.charge_mode_map = self.food_map * 0
        self.bot_map = None
    ...
    def _populate_bot_map(self):
        ...
        if bot_map_style == "existing_map":
            ...
            if bot_map_style == "new_random_map":
                ...
                _bots = np.argwhere(
                            np.ma.make_mask(
                                self.bot_map)[a:b , a:b])

                while len(_bots) != n_agents_on_food:
                    ...
                    print("n_bots on food",len(_bots))
                    # print("n_bots", len(_total_bots))
                    # if len(_bots) > n_agents_on_food:
                    #
                    #     self.bot_map[11:19, 11:19][random.choice(_bots)] = None
                    #     no_bots = np.argwhere((np.ma.make_mask(
                    #                             self.bot_map)*1)==0)
                    #     self.bot_map[random.choice(no_bots)] = Bot(
                    #         self, **BOT_DEFAULTS)
                    #
                    # if len(_bots) < n_agents_on_food:
                    #
                    ...
                    #             self.bot_map))
                    #
                    #     self.bot_map[random.choice(_bots)] = None
                    #     no_bots = np.argwhere(
                    #         np.ma.make_mask(
                    #             self.bot_map)[11:19 , 11:19]==0)
                    #
                    #     self.bot_map[random.choice(no_bots)] = Bot(
                    #         self, **BOT_DEFAULTS)
                    #
                    # _bots = np.argwhere(
                    #     np.ma.make_mask(
                    #         self.bot_map)[11:19, 11:19])
                    #
                    # _total_bots = np.argwhere(
                    #     np.ma.make_mask(
                    #         self.bot_map))
            ...
            # _food = np.argwhere((np.ma.make_mask(self.food_map) * 1 > 0)
            # bots_on_food = np.where((np.ma.make_mask(self.bot_map)*1)[_food])
            # if bots_on_food > n_agents_on_food:
            #     random.choice(np.ma.make_mask(self.bot_map)*1)[_food] = False
            #     padding = np.maximum(self.transmission_range,self.sense_range)
            #     random.choice(self.bot_map[padding:len(self.bot_map)-padding)]) = Bot(self, **BOT_DEFAULTS)



            if len(np.ma.make_mask(self.bot_map)[np.where(self.food_map)])>1:
                np.ma.make_mask(self.bot_map) * 1 > 0
                print("length", len(np.ma.make_mask(self.bot_map)[np.where(self.food_map)]))
                pass
    ...
    #     Produces _food_consumed_map of all energy accumulated from feeding
    #     Updates the stored energy map with the energy from feeding
    #
    #     """
    #     _food_units_consumed = BOT_DEFAULTS["e_consumed_per_feed"]
    #
    #     np.clip(self.food_map -
    #             (np.ma.make_mask(self.bot_map) * _food_units_consumed),
    #             0, float("inf"), self.food_map)
    #
    #     _food_consumed_map = np.clip(divide_matrices(self.food_map,
    #             np.ma.make_mask(self.bot_map)),
    #             0, 1)
    #
    #     self.Estored_map = np.clip(self.Estored_map +
    #         (_food_consumed_map * _food_units_consumed),
    #         0, BOT_DEFAULTS["e_stored_max"], self.Estored_map)
    ...
    def feed(self):
        ...
        """
        _food_units_consumed = BOT_DEFAULTS["e_consumed_per_feed"]
        _conversion_efficiency = BOT_DEFAULTS["energy_conversion_efficiency"]
        _time_steps_to_charge = BOT_DEFAULTS["time_steps_to_charge"]

        _new_charging_bots_map = np.multiply((np.ma.make_mask(self.food_map) * 1),
                                    (np.ma.make_mask(self.bot_map)
                                      - np.ma.make_mask(self.charge_mode_map)))

        self.food_map -= _new_charging_bots_map * _food_units_consumed

        # np.clip((self.food_map - _new_charging_bots_map * _food_units_consumed),
        #          0, float("inf"), self.food_map)

        # self.charge_mode_map += _new_charging_bots_map * _conversion_efficiency \
        #                         * _food_units_consumed / _time_steps_to_charge


        self.charge_mode_map += _new_charging_bots_map

        np.add(self.stored_energy_map, (self.charge_mode_map
                                        * _conversion_efficiency
                                        * _food_units_consumed
                                        / _time_steps_to_charge))


        # _food_consumed_map = np.clip(
        #     divide_matrices(self.food_map,
        #                     (np.ma.make_mask(self.bot_map)
        #                      - np.ma.make_mask(self.charge_mode_map))), 0, 1)
        #
        # self.Estored_map = np.clip(self.Estored_map +
        #                                  (
        #                                  _food_consumed_map * _food_units_consumed),
        #                                  0, BOT_DEFAULTS["e_stored_max"],
        #                                  self.Estored_map)
    ...
    def charge(self):
        ...
        self.stored_energy_map = np.add(self.stored_energy_map,
        ...
        print("")
        print("stored energy")
        print(self.stored_energy_map)
        print("store")
        print(np.ma.make_mask(self.store_mode_map)*1)
        print("explore")
        print(np.ma.make_mask(self.explore_mode_map) * 1)
        print("bots")
    ...
    def state_transition(self):
        ...
        transition_to_store =  np.where((np.ma.make_mask(self.bot_map)*1>0) &
                                        (self.stored_energy_map <=
                                         BOT_DEFAULTS["e_threshold_store"]) &
                                         (np.ma.make_mask(self.store_mode_map)
                                          * 1 < 1))
        ...
        # remove any transition to store bots from relay chains
        ...
        print("transition to explore =", transition_to_explore)

        self.explore_mode_map[transition_to_explore] = \
            self.bot_map[transition_to_explore]
        self.store_mode_map[transition_to_explore] = None
        self.charge_mode_map[transition_to_explore] = 0
    ...
    def feed_hungry_neighbour(self, location_bot_feed, location_bot_receive):
        ...
        if self.stored_energy_map[location_bot_feed[::-1]] \
            ...
            _e_donor = copy.copy(
                int(self.stored_energy_map[location_bot_feed[::-1]]))
    ...
    def save_data_init(self, food_map):
        ...
        self.save_data_per_timestep(bots_alive_before_move,
                                    # 0,
                                    # 0,
                                    _food_cells,
                                     nbots_walk_search,
...
def main():
    # ### 1) setup global variables and counters
    global count
    # global trophallaxis_count_explore
    # global trophallaxis_count_store
    # global food_cells_depleted_count
    ...
    # global transition_to_explore_count
    #
    # ## counters to compare with probabilistic model
    # trophallaxis_count_explore = 0
    # trophallaxis_count_store = 0
    # food_cells_depleted_count = 0
    # forage_success_count_explore = 0
    ...
    ### 4) save initial data and plot the map
    ...
    while (np.any(world.stored_energy_map) and np.any(world.food_map)):
        ...
        ### 5) reset counters to zero at start of each loop
        trophallaxis_count_explore = 0
        trophallaxis_count_store = 0
        food_cells_depleted_count = 0
        forage_success_count_explore = 0
        ...
        incapacitated_bots = world.state_transition()
        bots = np.copy(world.bot_map)
        bots[incapacitated_bots] = None
        functioning_bots = np.argwhere(bots)
            ...
            for functioning_bot in functioning_bots:
                ...
                if neighbours == []:
                    ...
                    if not world.food_map[location[::-1]]:
                        print("no food or neighbours, random step")
                        world.setup_move("new_location_random",
                                         location, None,
                                         np.clip(world.stored_energy_map[
                                                         location[::-1]],
    ...
    # create a file to mark the end
    # completed = dirname + "/" + subdirname + "/" \
    #                + subdirname + '_completed.txt'


    #world.plot_data_per_time_step(_bots_with_Estored)
    ...
    print("food_remaining:", np.sum(world.food_map))
