/*
 * DVOLUME in WaveRP.h must be nonzero to compile this example
 *
 * This example demonstrates software volume control.  It starts playing
 * each file with 37 db attenuation and increases the volume to no attenuation
 * then jumps to 37 db attenuation.
 */
#include <WaveRP.h>
#include <SdFatUtil.h>
#include <SdFat.h>
    ...
    if (file.isDir()) {                   // check if we opened a new directory
      ...
      if (!wave.play(&file)) {            // Figure out, is it a WAV proper?
        ...
        while (wave.isPlaying()) {// playing occurs in interrupts, so we print dots in realtime
          ...
          if (!(n % 2)){
            wave.setVolume(v);
            //  increase by one db or go to 37 db down
            v = v > 0 ? v - 1 : 37;
          }
        }
        sdErrorCheck();                    // everything OK?
        if (wave.busyError)Serial.println(wave.busyError, DEC);     // wave decoding errors
      }
    }
  }
}